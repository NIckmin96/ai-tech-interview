...
이러한 정리가 유용한 이유는, 모집단의 분포에 상관 없이(모집단의 분포가 한쪽으로 쏠려있어도) 표본평균의 분포가 근사적으로 정규분포를 따르며, 이 사실을 이용하여 모평균에 대한 통계적 추정과 검정이 가능해지기 때문이다. 즉, 모집단 전체를 조사하지 않아도 표본을 여러개 뽑을 수 있다면, 그 표본들의 평균을 이용해서 모집단의 평균과 분산을 추정할 수 있는 것이다. 
