...
$$
n=\frac{n_0}{1+\frac{(n_0-1)}{N}}=\frac{N*n_0}{n_0+N-1}
$$
로 필요한 표본의 크기를 계산할 수 있다. (수정된 Chochran 공식)

예를 들어, 신뢰구간 95% 내에 남녀비율이 50:50인 모집단을 올바르게 대표할 수 있는 표본의 크기를 계산한다고 하자.
$$
n_0 = \frac{{Z_{\frac{0.95}{2}}}^2p(1-p)}{e^2} = \frac{1.96^2\times0.5\times(1-0.5)}{(1-0.95)^2}=384.15
$$
로 계산되어 385명 이상의 표본이 필요하다는 것을 알 수 있게 된다. 이를 두번째 공식으로 조정하게 된다면(인구 수가 10만명이라고 가정),
$$
n=\frac{n_0}{1+\frac{(n_0-1)}{N}}=\frac{384.15}{1+\frac{(384.15-1)}{100000}}=382.68
$$
로 수정된다.
...
![베르누이](https://datascienceschool.net/_images/08.02%20%EB%B2%A0%EB%A5%B4%EB%88%84%EC%9D%B4%EB%B6%84%ED%8F%AC%EC%99%80%20%EC%9D%B4%ED%95%AD%EB%B6%84%ED%8F%AC_14_0.png)
**이항분포**는 베르누이 시행을 N번 (복원) 반복하였을 때 성공이 나타난 횟수를 확률변수로 하는 확룰 분포이다.
![바이노미얼](https://datascienceschool.net/_images/08.02%20%EB%B2%A0%EB%A5%B4%EB%88%84%EC%9D%B4%EB%B6%84%ED%8F%AC%EC%99%80%20%EC%9D%B4%ED%95%AD%EB%B6%84%ED%8F%AC_38_0.png)

**카테고리분포**와 **다항분포**는 각각 베르누이분포와 이항분포의 확장판이라고 볼 수 있다. **카테고리 확률 변수**는 결과값으로 1부터 K까지 K개 정수값 중 하나를 가진다고 할 때, 0 또는 1로만 이루어진 다차원 벡터를 출력하는 것이 특징이기도 하다. (예/ x = (1,0,0,0,0,0))
결과값으로 1부터 K까지 K개 정수값 중 하나를 가지는 실험을 시행했을 때의 **카테고리분포**의 확률질량함수는 다음과 같이 생겼다.
![카테고리](https://datascienceschool.net/_images/08.03%20%EC%B9%B4%ED%85%8C%EA%B3%A0%EB%A6%AC%EB%B6%84%ED%8F%AC%EC%99%80%20%EB%8B%A4%ED%95%AD%EB%B6%84%ED%8F%AC_15_0.png)
결과값으로 1부터 K까지 K개 정수값 중 하나를 가지는 실험을 N번 반복 시행했을 때의 **다항분포**의 시뮬레이션 예시는 다음과 같다.
![멀티노미얼](https://datascienceschool.net/_images/08.03%20%EC%B9%B4%ED%85%8C%EA%B3%A0%EB%A6%AC%EB%B6%84%ED%8F%AC%EC%99%80%20%EB%8B%A4%ED%95%AD%EB%B6%84%ED%8F%AC_36_0.png)
...
![노말](https://datascienceschool.net/_images/08.04%20%EC%A0%95%EA%B7%9C%EB%B6%84%ED%8F%AC%EC%99%80%20%EC%A4%91%EC%8B%AC%EA%B7%B9%ED%95%9C%EC%A0%95%EB%A6%AC_6_0.png)
...
![t](https://mblogthumb-phinf.pstatic.net/MjAxNjExMDVfMjAy/MDAxNDc4Mjc2NDkyNTAy.zX3mW-IUzPwia1fiXjyGdqaWBPxoIGvGS3-DsOWOC0Ig.2SyHg7SPXvf_NalroaenfpGv-VlmrtyRTMJZPCeLfVAg.JPEG.mykepzzang/%ED%94%84%EB%A0%88%EC%A0%A0%ED%85%8C%EC%9D%B4%EC%85%981.jpg?type=w2)
**카이제곱분포** (설명 추가 필요)
![카이스퀘어드](https://mblogthumb-phinf.pstatic.net/MjAxNjExMDNfMjk1/MDAxNDc4MTA4MDUzMjE5.MRS0p0mLkK4K6q2p3O6XuCKHwr7Z1PQGD0Ha4qojQ9kg.5XmeIvpkyY7kdRikQzStvp2MFqo-ZL_toxsdDJIOOM0g.JPEG.mykepzzang/%ED%94%84%EB%A0%88%EC%A0%A0%ED%85%8C%EC%9D%B4%EC%85%981.jpg?type=w2)
**F 분포** (설명 추가 필요)
...
$$
P(B) = P(A \cap B)+P(A^c \cap B) = (\frac{2}{3}\times\frac{2}{3}\times\frac{2}{3})+(\frac{1}{3}\times\frac{1}{3}\times\frac{1}{3})
$$
*세명이 모두 비가 내린다고 말을 했을 때 실제 비가 내릴 경우의 조건부확률*은 다음과 같다.
$$
P(A|B) = \frac{P(A \cap B)}{P(B)} = \frac{\frac{2}{3}\times\frac{2}{3}\times\frac{2}{3}}{(\frac{2}{3}\times\frac{2}{3}\times\frac{2}{3})+(\frac{1}{3}\times\frac{1}{3}\times\frac{1}{3})} = \frac{8}{9}
$$
