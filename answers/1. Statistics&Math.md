...
<img src="https://latex.codecogs.com/svg.image?R^2" title="R^2" />(결정계수)는 선형 회귀 모델의 분산 설명력이라고 볼 수 있다. 다시 말해, 우리가 만든 모델(회귀선)이 얼마나 데이터를 잘 설명하는지를 의미하는 것이다.
...
중심극한정리란 모집단에서 표본추출이 무수히 많이 되는 경우(일반적으로 30번 이상), 표본평균 <img src="https://latex.codecogs.com/svg.image?\overline{X}" title="\overline{X}" />의 분포가 정규분포에 가까워진다는 것이다. 
