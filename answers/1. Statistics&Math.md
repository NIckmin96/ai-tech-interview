...
$R^2$(결정계수)는 선형 회귀 모델의 분산 설명력이라고 볼 수 있다. 다시 말해, 우리가 만든 모델(회귀선)이 얼마나 데이터를 잘 설명하는지를 의미하는 것이다.

- $R^2$ 계산법
  - $R^2$ = $1 -\frac{SSE}{SST}$  
    이때, 
    - SST(Total Sum of Squares) = $\sum(y_i-\bar{y}_i)^2$ (관측값에서 관측값의 평균을 뺀 결과의 총합)
    - SSE(Error Sum of Squares) = $\sum(y_i-\hat{y}_i)^2$ (관측값에서 추정값을 뺀 값. 즉 잔차(residual)의 총합)  
...
데이터의 분포에 따라 평균과 중앙값이 결정될 수 있다. 모든 데이터를 고려한 값인 평균이 일반적으로 중심위치의 척도로 가장 먼저 선택된다. 하지만 아주 큰 관측값이나 아주 작은 관측값(outlier)에 영향을 많이 받는다.
...
중심극한정리란 모집단에서 표본추출이 무수히 많이 되는 경우(일반적으로 30번 이상), 표본평균 $\overline{X}$의 분포가 정규분포에 가까워진다는 것이다. 
...
* 10개의 동전 중 500원짜리 5개, 100원짜리 5개가 있을 경우  
  $E(5,5)=-\frac{5}{10}log_2\frac{5}{10}-\frac{5}{10}log_2\frac{5}{10}=1$  

* 10개 동전 중 500원짜리가 10개 있을 경우  
  $E(10)=-\frac{10}{10}log_2\frac{10}{10}=0$
...
즉 확률은 아래의 식으로 계산된다.  
  * $pr($ 32~34의 몸무게를 가지는 경우(변동 가능) | $N(32, 2.5^2)$ $)$ 

다른 몸무게에 대해 확인해보고 싶다면 식에서 오른쪽의 조건을 변경해주면 된다. 하지만 왼쪽의 분포 조건은 바뀌지 않는다.

</br>

반면 가능도에서는 이미 특정 쥐에 대한 몸무게를 측정했다고 가정한다. 몸무게가 34그램인 쥐가 관측되었다면, $N(32, 2.5^2)$에서 이 데이터에 해당하는 값은 0.12이고 이것이 가능도가 된다.

이때 정규분포를 오른쪽으로 이동하여 $N(34, 2.5^2)$를 따르게 한다면, 이에 해당하는 가능도는 0.21이 된다.

</br>

마지막으로 식을 정리하면 아래와 같다. 

확률: $pr($ data(변동 가능) | distribution(고정됨) $)$  
가능도: $L($ distribution(변동 가능) | data(고정됨) $)$
