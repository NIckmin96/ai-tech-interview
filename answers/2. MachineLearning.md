...
이때, 정규화 방법에는 대표적으로 L1 정규화와 L2 정규화가 있다. 두 정규화는 모델 학습시에 값이 너무 큰 파라메터의 영향력을 줄인다. L1 정규화는 Cost Function에 가중치의 크기(절대값)를 더해주고, L2 정규화는 가중치 크기의 제곱을 더해줌으로써 가중치가 너무 크지 않은 방향으로 학습을 유도한다. 
