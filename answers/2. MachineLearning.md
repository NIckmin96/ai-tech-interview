...
- **차원의 확장**
  ...
  <img src="C:\Users\Administrator1\AppData\Roaming\Typora\typora-user-images\image-20220126184423115.png" alt="image-20220126184423115" style="zoom:50%;" />
...
- **조건부 독립을 가정하는 이유**
  ...
  <img src="../../../../AppData/Roaming/Typora/typora-user-images/image-20220126202222327.png" alt="image-20220126202222327" style="zoom:67%;" />

  위 그림에서 정답 label (EnjoySpt)를 추정하기 위해서는 모든 feature들(sky, Temp, Humid, ...)을 고려해야 한다. 만약 조건부 독립이 아니라면 
  $$
  P(x_1=sunny, x_2=warm, ...|y=Yes)
  $$
  위 수식을 계산하기 위해서는 복잡한 식이 필요하다. 

  하지만, 조건부 독립을 가정한다면
  $$
  P(x_1=sunny|y=Yes)*P(x_2=warm|y=Yes)...
  $$
  로 식을 간소화할 수 있다. 
...
- **분류문제**
  ...
  - CE (Cross Entropy)
    ...
    $$
    CE = -\sum_kt_k\log{y_k}\\
    (t : one\;hot\; encoding\; vector,\;\; y: output)
    $$
  ...
  - CCE (Categorical Cross Entropy)
    ...
    $$
    CCE=-\frac {1}{N}\sum_{i=0}t_k\log{y_k}+(1-t_k)\log{(1-y_k)}
    $$
